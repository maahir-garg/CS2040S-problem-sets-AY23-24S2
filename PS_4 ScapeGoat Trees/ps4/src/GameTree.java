...
public class GameTree {
     ...
     */
    int findValue() {
        return findValueHelper(root, true);
    }

    // Recursively setting values in favour of PlayerOne or PlayerTwo
    public int findValueHelper(TreeNode node, boolean isPlayerOne) {
        if (node == null) {
            ...
            if (isPlayerOne) {
                ...
                for (TreeNode child : node.children) {
                    int val = findValueHelper(child, false);
                    if (current_max < val) {
                        current_max = val;
                    }
                }
                node.value = current_max;
                return node.value;
            } else {
                int current_min = Integer.MAX_VALUE;
                for (TreeNode child : node.children) {
                    int val = findValueHelper(child, true);
                    if (current_min > val) {
                        current_min = val;
                    }
                }
                node.value = current_min;
                return node.value;
            }
